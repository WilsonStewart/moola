...
func (d *Datafile) OpenAccount(node OpenAccountNode) error {
	...
	if slices.Contains(d.Symbols.AccountNames, node.requestedAccountName) {
		return fmt.Errorf(
	...
	if slices.Contains(d.Symbols.AccountAliasNames, node.requestedAccountName) {
		return fmt.Errorf(
	...
	d.Accounts[node.requestedAccountName] = &Account{
		Name:    node.requestedAccountName,
		Kind:    node.accountKind,
		Balance: 0,
	}
	d.Symbols.AccountNames = append(d.Symbols.AccountNames, node.requestedAccountName)

	if node.requestedAccountAlias != nil {
		if slices.Contains(d.Symbols.AccountAliasNames, *node.requestedAccountAlias) {
			return fmt.Errorf(
		...
		if slices.Contains(d.Symbols.AccountNames, *node.requestedAccountAlias) {
			return fmt.Errorf(
				"duplicate symbol: cannot create account alias %q as an account with that name already exists!",
				*node.requestedAccountAlias,
			)
		}
		d.AccountAliases[*node.requestedAccountAlias] = node.requestedAccountName
		d.Symbols.AccountAliasNames = append(d.Symbols.AccountAliasNames, *node.requestedAccountAlias)
	}
